        ...
        <tbody>
            ...
            while ($row = mysqli_fetch_array($sql)) {
                ...
                <tr class="single-item">
                    ...
                    <td>
                        <?php echo 'Rp ' . number_format($row['qty'] * $row['price'], 0, ',', '.') ?>
                    </td>
                    <td class="text-end">
                        <div class="hstack gap-2 justify-content-end">
                            <a href="javascript:void(0);" class="avatar-text avatar-md edit"
                                data-id="<?php echo $row['custom_product_id'] ?>" data-bs-toggle="tooltip" title="Edit"
                                data-bs-original-title="Edit">
                                <i class="feather-edit-3"></i>
                            </a>
                            <button href="javascript:void(0);" class="avatar-text avatar-md hapus" data-bs-toggle="tooltip"
                                title="Hapus" data-id="<?php echo $row['custom_product_id'] ?>"
...
<script>
    ...
    $('#table').on('click', '.hapus', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                console.log(id);
                $.ajax({
                    url: 'process.php?act=hapus-custom',
                    type: 'POST',
    ...
    $('#table').on('click', '.edit', function () {
        var id = $(this).data("id");
        $.ajax({
            url: 'pages/pesanan/edit-custom.php',
            method: 'post',
            data: { id: id },
            success: function (data) {
                $('#tampil_data').html(data);
                document.getElementById("judul").innerHTML = 'Edit Pesanan';
            }
        });
        $('#modal').modal('show');
    });
