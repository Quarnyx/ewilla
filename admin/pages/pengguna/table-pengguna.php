<div class="table-responsive">
    <table class="table table-hover" id="table">
        ...
        <tbody>
            ...
            $sql = mysqli_query($conn, "SELECT * FROM lp_users");
            $no = 0;
                    ...
                    <td>
                        <div class="hstack gap-2 justify-content-end">
                            <a href="javascript:void(0);" class="avatar-text avatar-md edit"
                                data-id="<?php echo $row['user_id'] ?>" data-bs-toggle="tooltip" title="Edit"
                                data-bs-original-title="Edit">
                                <i class="feather-edit-3"></i>
                            </a>
                            <button href="javascript:void(0);" class="avatar-text avatar-md hapus" data-bs-toggle="tooltip"
                                title="Hapus" data-id="<?php echo $row['user_id'] ?>" data-bs-original-title="Hapus">
                                <i class="feather-trash-2"></i>
                            </button>
                            <button href="javascript:void(0);" class="avatar-text avatar-md password"
                                data-bs-toggle="tooltip" title="Ganti Password" data-id="<?php echo $row['user_id'] ?>"
                                data-bs-original-title="Ganti Password">
...
<script>
    ...
    $('#table').on('click', '.password', function () {
        var id = $(this).data("id");
        $.ajax({
            url: 'pages/pengguna/edit-password.php',
            method: 'post',
            data: { id: id },
            success: function (data) {
                $('#tampil_data').html(data);
